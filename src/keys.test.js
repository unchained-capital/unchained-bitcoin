...
const extendedPublicKeyConversions = TEST_FIXTURES.nodes["m/45'/0'/0'"]
...
describe("keys", () => {
  ...
  describe("Test convertExtendedPublicKey", () => {
    Object.keys(extendedPublicKeyConversions).forEach((convertTo) => {
      describe(`Test converting to ${convertTo}`, () => {
        Object.keys(extendedPublicKeyConversions).forEach((convertFrom) => {
          if (convertFrom === convertTo) return;
          it(`should properly convert extended public key from ${convertFrom} to ${convertTo}`, () => {
            const converted = convertExtendedPublicKey(extendedPublicKeyConversions[convertFrom], convertTo)
            expect(converted).toBe(extendedPublicKeyConversions[convertTo])
          });
